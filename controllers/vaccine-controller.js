...
exports.get_all_posts = function(req, res){
    ...
    let sql = "select u.userId, u.userName, c.cityId, name as cityName, facility, content, dateCreated from Posts p" + 
...
exports.get_posts_by_city = function(req, res){
    res.setHeader('Access-Control-Allow-Origin','*');
    let sql = "select u.userId, u.userName, c.cityId, name as cityName, facility, content, dateCreated from Posts p inner join Users u on u.userId = p.userId " + 
...
exports.post_data = function(req, res){
    ...
    const facility = req.body.facility.replace(/'/g, "''");
    let sql = "call createPost('" + name + "', '" + city + "', '" + facility + "', '" + comments + "', '" + dateLocal + "')";
    con.query(sql, function (err, result) {
        if (err) throw err;
        res.send(result);
    });
